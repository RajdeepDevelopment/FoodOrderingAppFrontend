...
import { currentMeetingResetAsync, getAllAppliedDataAsync, getAllAppliedDataForSearchAsync, getNewMeetPosttAsync, getUpdateApplyJobAsync, getUpdateUserAddressAsync, getdeleteApplyJobAsync, selectApply, selectProducts, selectcurrentMeetingLink } from "../../features/counter/counterSlice";
...
function AppliedJob(){
  ...
  const [meetingDateTime, setMeetingDateTime] = useState("2023-11-20T07:19:09Z");
  const [meetingData, setMeetingData] = useState({
    ...
    const [sheduleBarIndex, setSheduleBarIndex] = useState(-1); 
...
function SheduleMeet (newMeet,appliedJob){
...
const sendEmail = async (info, currentMeetingLinkk ) => {
  ...
  try { 
    const templateParams = { 
      ...
      sendmailto: "rajdeepsadhu17@gmail.com"
...
const AppliedJobData = useSelector(selectApply)

useEffect(()=>{

dispatch (getAllAppliedDataAsync())

}, [])
 
function ReqAccept(  position,id){
    const postData = { Access: position , id: id}
    Swal.fire({
      position: 'top-end',
      icon: 'success',
...
<div className="col-8"> 
<h2 className="bhag display-6    "> Admin dashboard displays all pending job requests for review and assignment.</h2>
</div>
</div>
                  ...
                  <div className="row">
                    <div className="col-lg-6">
                      <div className="records">
                        Showing: <b>1-20</b> of <b>{AppliedJobData?.length}</b> result
                      </div>
                    </div>
                    <div className="col-lg-6">
                ...
                <div className="row no-gutters">
                  <div className="col-lg-3 col-md-3 col-sm-12 p-0">
                    <select
                      ...
                      onChange={(e)=>{
                        dispatch(getAllAppliedDataForSearchAsync(e.target.value))
                      }}
                  ...
                  dispatch(getAllAppliedDataForSearchAsync(searchBarData)); 
                  }}>
                  <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width={24}
                ...
                <div className="result-body">
                  <div className="table-responsive">
                    <table className="table widget-26">
                      <tbody style={{ height: '100px', width: '100%', overflowY: 'auto' }} >
                           {/* MP */}
  <div  style={{ height: '500px', width: '100%', overflowY: 'auto' }}> 

           { AppliedJobData.length !=0 && AppliedJobData.map((appliedJob, index)=>(
 <tr className="bhag">

{ sheduleBarIndex === index && 
 <Draggable>

<div key={index+appliedJob["First-name"]} style={containerStyle}> 
<div className="widget-26-job-title bhag mb-3 p-4">
     <div href="#">
       Job Profile :  {appliedJob.title}
     </div>
     <p className="m-0">
       <a href="#" className="employer-name mx-2">
       {appliedJob["First-name"]} {" "} {appliedJob["Last-name"]}
       </a>{" "}
 ...
 {currentMeetingLink.length ===0 &&
  <form onSubmit={ handleSubmit((data)=>{
        ...
        SheduleMeet(newMeet,appliedJob ); 
         reset();
          ...
          <button type="" className="btn btn-primary mx-4" onClick={()=>{
            setSheduleBarIndex(-1)
          }}>
            Close
          </button>
        </form>
        ...
        {currentMeetingLink.length !== 0 && currentMeetingLink[0] && (
          <> 


  <div className="card-body ">
    <h5 className="card-title mx-2">Meeting Topic: {currentMeetingLink[0]?.topic}</h5>
    <p className="card-text mx-2"><strong>Meeting ID:</strong> {currentMeetingLink[0]?.id}</p>
    <p className="card-text mx-2"><strong>Start Time:</strong> {currentMeetingLink[0]?.start_time}</p>
    <p className="card-text mx-2"><strong>Duration:</strong> {currentMeetingLink[0]?.duration} minutes</p>
    <p className="card-text ">
      <strong>Join URL:</strong>{' '}
      <a href={currentMeetingLink[0]?.join_url} target="_blank" rel="noopener noreferrer">
        Join Meeting
      </a>
    </p>
  </div>
<din className =""> 
  <button className="btn btn-primary btn-sm " onClick={()=>{
           sendEmail(appliedJob, currentMeetingLink[0])
           setSheduleBarIndex(-1)
           Swal.fire({
            position: "center",
            icon: "success",
          ...
          dispatch(getUpdateApplyJobAsync({id:appliedJob.id, status: "schedule" }))

  }}> Shedule Meeting</button>
  </din>
          </>
 ...
 <td>
   <div className="widget-26-job-info">
     ...
     <p className="text-muted m-0">
       in <span className="location">{appliedJob.location}</span>
     </p>
   </div>
 </td>
 <td>
   <div className="widget-26-job-salary">{appliedJob.salary}</div>
 </td>
 <td>
   <div className="widget-26-job-category bg-soft-base mx-2">
   <span >{appliedJob.title}</span>
 ...
 <td>
   <div className="widget-26-job-starred">
    ...
    {appliedJob.status ==="pending" &&
      <button type="button" class="btn btn-danger btn-sm mx-3" onClick={()=>{
  setSheduleBarIndex(index); 
   ...
   <button type="button" class="btn btn-danger btn-sm mx-3 " onClick={()=>{
    dispatch(getdeleteApplyJobAsync( {id: appliedJob.id, index: index}))
   }}>Remove</button>

   <button type="button" class="btn btn-primary btn-sm mx-3" onClick={()=>{

    ReqAccept(appliedJob.title, appliedJob.id); 
    dispatch(getdeleteApplyJobAsync( {id: appliedJob.id, index: index}))
